...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train/evaluate linear approximation Q-learning.")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("results/approx_q_learning"))
...
def maybe_plot_learning_curve(history: List[Dict[str, float]], out_path: Path) -> str:
    ...
    ax1.plot(epochs, test_returns, label="Eval Test Return", color="#fd7e14", linewidth=2.0)
...
def main() -> None:
    ...
    feature_dim = int(result["feature_dim"])  # type: ignore[arg-type]

    weights: np.ndarray = result["weights"]  # type: ignore[assignment]
    best_weights: np.ndarray = result["best_weights"]  # type: ignore[assignment]

    history_csv = output_dir / "history.csv"
    summary_json = output_dir / "summary.json"
    weights_npy = output_dir / "weights_final.npy"
    best_weights_npy = output_dir / "weights_best.npy"
    curve_png = output_dir / "learning_curve.png"

    write_history_csv(history_csv, history)
    np.save(weights_npy, weights)
    np.save(best_weights_npy, best_weights)
    ...
    summary = {
        ...
        "artifacts": {
            ...
            "weights_final_npy": str(weights_npy),
            "weights_best_npy": str(best_weights_npy),
    ...
    print(f"Wrote {weights_npy}")
    print(f"Wrote {best_weights_npy}")
