...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run reward sensitivity experiments.")
    ...
    parser.add_argument("--methods", type=str, default="approx")
    parser.add_argument(
        "--reward-grid",
        type=str,
        default="0.8:0.6:2.0,1.0:0.6:2.0,1.2:0.6:2.0,1.0:0.4:2.0,1.0:0.8:2.0",
        help="Comma-separated alpha:beta:gamma triples.",
    )
    ...
    parser.add_argument("--approx-num-epochs", type=int, default=200)
    ...
    parser.add_argument("--output-prefix", type=Path, default=Path("results/reward_sensitivity"))
...
def parse_methods(raw: str) -> List[str]:
    ...
    for token in raw.split(","):
        ...
        if method not in allowed:
            raise ValueError(f"Unsupported method '{method}', choose from {sorted(allowed)}")
...
def parse_reward_grid(raw: str) -> List[Tuple[float, float, float]]:
    ...
    for token in raw.split(","):
        ...
        parts = token.split(":")
        if len(parts) != 3:
            raise ValueError(f"Invalid reward triple '{token}', expected alpha:beta:gamma")
        triples.append((float(parts[0]), float(parts[1]), float(parts[2])))
    if not triples:
        raise ValueError("Reward grid is empty.")
    return triples
...
def aggregate(runs: Sequence[Dict[str, object]]) -> Dict[str, Dict[str, float]]:
    ...
    for metric in METRIC_KEYS:
        values = [float(run["best_test_metrics"][metric]) for run in runs]  # type: ignore[index]
        out[metric] = {"mean": float(np.mean(values)), "std": float(np.std(values, ddof=0))}
...
def main() -> None:
    ...
    reward_grid = parse_reward_grid(args.reward_grid)
    ...
    if not train_episodes or not test_episodes:
        raise RuntimeError("Not enough train/test episodes for reward sensitivity run.")

    records: List[Dict[str, object]] = []
    rows: List[Dict[str, object]] = []

    for method in methods:
        for alpha_energy, beta_latency, gamma_miss in reward_grid:
            env_config = replace(
                DEFAULT_ENV_CONFIG,
            ...
            print(
                ...
                f"reward=({alpha_energy:.2f},{beta_latency:.2f},{gamma_miss:.2f}) "
                f"return={stats['avg_episode_return']['mean']:.3f}+/-{stats['avg_episode_return']['std']:.3f}"
            )

    rows.sort(
        key=lambda r: (str(r["method"]), -float(r["avg_episode_return_mean"])),
    )
    ...
    payload = {
        "config": {
            ...
            "methods": methods,
            "reward_grid": [
                {"alpha_energy": a, "beta_latency": b, "gamma_miss": g}
                for a, b, g in reward_grid
            ],
        },
        "records": records,
        "rows": rows,
